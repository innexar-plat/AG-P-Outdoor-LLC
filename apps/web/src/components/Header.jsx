  ...
  return (
    <header
      className={`font-header fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-[#0f1923] shadow-2xl shadow-black/50 py-2'
          : 'bg-gradient-to-r from-[#0f1923]/90 to-[#132010]/90 backdrop-blur-lg py-4'
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 hidden sm:block font-normal tracking-wide">{site.tagline}</p>
            </div>
          </Link>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden mt-4 pb-4 border-t border-white/15"
            >
              <div className="flex flex-col space-y-3 pt-4">
                ...
                <div className="border-t border-white/15 pt-3">
                  <p className="text-xs font-semibold text-white/40 mb-2">Services</p>
                  {services.map((service) => (
                    <Link
                      key={service.path}
                      to={service.path}
