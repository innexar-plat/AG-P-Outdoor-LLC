...
export async function PUT(
  ...
  try {
    const row = await updatePortfolioCategory(id, parsed.data);
    return NextResponse.json({ data: row, error: null });
  } catch (err) {
    console.error("[admin/portfolio/categories/[id] PUT]", err);
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[admin/portfolio/categories/[id] DELETE]", err);
    return NextResponse.json({ data: null, error: "Something went wrong" }, { status: 500 });
  }
}
